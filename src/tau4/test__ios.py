...
from tau4.ios.hil.arduino import ArduinoNano
...
class _TESTCASE__hil_arduino(unittest.TestCase):
    ...
    def test__normal_use( self):
        ...
        if 1:
            ...
            pin_spec = DOutSpec( p_raw=pandora.BoxClippingMonitored( id="led1", value=0, label="LED 1", dim="", bounds=(0, 1)))
            pin_impl = ArduinoDOUT( nano1, "d2")
            pin_spec.connect( pin_impl)
            IOSpecPool().douts().add( pin_spec)
                                            # Im IOSpecPool können auch Pins sein,
                                            #   die zum LabJack usw. gehören.
            pin_spec = DOutSpec( p_raw=pandora.BoxClippingMonitored( id="led2", value=0, label="LED 2", dim="", bounds=(0, 1)))
            pin_impl = ArduinoDOUT( nano1, "d13")
            pin_spec.connect( pin_impl)
            IOSpecPool().douts().add( pin_spec)
                                            # Im IOSpecPool können auch Pins sein,
                                            #   die zum LabJack usw. gehören.
            ...
            pin_spec = DInpSpec( p_raw=pandora.BoxClippingMonitored( id="button", value=0, label="Button", dim="", bounds=(0, 1)))
            pin_impl = ArduinoDINP( nano1, "d3")
            pin_spec.connect( pin_impl)
            IOSpecPool().dinps().add( pin_spec)

            IOSpecPool().execute_inps()
            value = IOSpecPool().dinps( "button").value()
            print( "Button state = '%d'. " % value)
