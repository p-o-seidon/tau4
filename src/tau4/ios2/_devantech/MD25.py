...
from tau4.ios.hil import MDImpl
...
class _MD25 :
    ...
    def __init__( self, address=0x58, mode=1, debug=False):
        from tau4.ios.hil.devantech.Adafruit_I2C import Adafruit_I2C
...
class MD25(MDImpl, _MD25):

    def __init__( self, id, address=0x58, mode=1, debug=False):
        MDImpl.__init__( self, id, is_direction_inverted=True)
    ...
    def speed_100( self, lhs, rhs):
        ...
        if lhs is not None:
            ...
            self.i2c.write8( self._MD25_SPEED_1, LHS)

        if rhs is not None:
            rhs = rhs/100*self.speed_max_100()
            RHS = int( 255/200*rhs + 127.5)
            self.i2c.write8( self._MD25_SPEED_2, RHS)

        return


class MD25Dummy(MDImpl):

    def __init__( self, id, address=0x58, mode=1, debug=False):
        MDImpl.__init__( self, id)
        return
