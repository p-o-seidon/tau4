...
cdef class Matrix3x3:
    ...
    def __mul__( self, other):
        ...
        if isinstance( other, V3D):
            ...
            return V3D( self[ 1, 1]*v1 + self[ 1, 2]*v2 + self[ 1, 3]*v3, self[ 2, 1]*v1 + self[ 2, 2]*v2 + self[ 2, 3]*v3, self[ 3, 1]*v1 + self[ 3, 2]*v2 + self[ 3, 3]*v3)
...
cdef class V3D:
    ...
    def __add__( self, other):
        ...
        if isinstance( other, tuple):
            ...
            return V3D( v1 + w1, v2 + w2, v3 + w3)

        if isinstance( other, (float, int)):
            return V3D( v1 + other, v2 + other, v3 + other)

        try:
            w1, w2, w3 = other.elems()

        except AttributeError:
            raise TypeError( "Adding a '%s' to a V3D doesn't seem to make sense!" % type( other))
    ...
    def __richcmp__( self, other, op):
        if not isinstance( other, V3D):
            raise TypeError( "Can only compare two V3D, but other is a '%s' (value: '%s')!" % (type( other), other))
    ...
    def __repr__( self):
        x, y, z = tuple( self.__elems)
        return u"%s( x=%.3f, y=%.3f, z=%.3f; len=%.3f)" % (self.__class__.__name__, x, y, z, self.mag())
    ...
    cpdef double    dot( self, other):
        if not isinstance( other, V3D):
            raise TypeError( "Can only dot two V3D, but other is a '%s' (value: '%s')!" % (type( other), other))

        x, y, z = self.__elems
        u, v, w = other.elems()
        return x*u + y*v + z*w

    cpdef V3D   ex( self, other):
        a1, a2, a3 = self.__elems
        b1, b2, b3 = other.elems()
        return V3D( a2*b3 - a3*b2, a3*b1 - a1*b3, a1*b2 - a2*b1)
...
cdef class R3D(Matrix3x3):
    ...
    @staticmethod
    def FromVectors( V3D ex, V3D ey, V3D ez):
...
cpdef R3D   R3DFromVectors( V3D ex, V3D ey, V3D ez):
    cdef:
        double  r11, r12, r13, r21, r22, r23, r31, r32, r33
...
cdef class T3D:
    ...
    def FromEuler( x=0, y=0, z=0, alpha=0, beta=0, gamma=0):
        return T3D( R3DFromEuler( alpha, beta, gamma), V3D( x, y, z))

    def __init__( self, Matrix3x3 R, V3D P):
        self.__R3D = R
                                        # Rotationsmatrix
        self.__p3D = P
                                        # Verschiebevektor
    ...
    def __richcmp__( self, other, op):
        if op == 2: # ==
            return self._P_() == other._P_() and self._R_() == other._R_()
    ...
    def __mul__( self, other):
        if isinstance( other, (V3D, V3D)):
            return V3D( *(self._R_() * other + self._P_()).elems())
    ...
    cpdef tuple     pose2D( self, dim=u"mm"):
        """2D-Teil der Pose.
