...
class T3D:
    ...
    def __mul__( self, other):
        if isinstance( other, (p3D, V3D, V3D)):
    ...
    def a( self, deg=False, no_neg=False):
        a = self.euler( deg)[ 3]
        if no_neg:
            if a < 0:
...
class R3D(Matrix3x3):
    ...
    @staticmethod
    def FromVectors( ex: V3D, ey: V3D, ez: V3D):
...
def R3DFromVectors( ex: V3D, ey: V3D, ez: V3D) -> R3D:
