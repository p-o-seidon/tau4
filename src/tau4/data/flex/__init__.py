...
class Variable:
    ...
    def __repr__( self):
        return "<Variable(id=%s: value=%s)/>" % ( self.id(), self.value())
...
class VarblCats(metaclass=Singleton):
    ...
    def varblcat( self, id):
        return self.__varblcats[ varblcat.id()]

    def varblcat_default( self):
        try:
            return self.__varblcats[ varblcat.id()]

        except KeyError:
            vc = VarblCat( "__default__")
                                        # Creates a VarblCat and adds it to this collection
            return vc
