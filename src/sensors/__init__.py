...
class NaviSensorDataEmlidReach(SensorData):
    ...
    def __init__( self, *, sensor):
        ...
        assert isinstance( self.__sensor, NaviEmlidReach)
        
        self.__count_sats = self.__sensor.satellite_count()
        self.__wX = self.__sensor.wP().x()
        self.__wY = self.__sensor.wP().y()
        self.__bX = self.__sensor.bP().x()
        self.__bY = self.__sensor.bP().y()
        self.__bA = 0
#        self.__hdist = hdist
#        self.__hspeed = hspeed
#        self.__mapurl = mapurl
...
#        self.__mapurl = other.__mapurl
    ...
    @overrides( SensorData)
    def clone( self):
        return self.__class__( sensor=self.sensor())
...
class NaviStatus(flex.VariableDeClMo):

    """Folgende Status sind möglich: NOK (= 0), O (= 1).
    
    2DO:
        Umbenennen nach GpsStatus.
    """
...
class Navis(list):
    ...
    def __init__( self):
        ...
        self.__tau4p_on_directionvector_modified = PublisherChannel.Synch( self)
    ...
    def reg_tau4s_on_directionvector_modified( self, tau4s):
        self.__tau4p_on_directionvector_modified += tau4s
        return
    
    def tau4p_on_directionvector_modified( self):
        self.__tau4p_on_directionvector_modified()
...
class Ranger2(Sensor3):
    ...
    def rPo( self):
        ...
        """
        rTs = self.rT()
                                        # Sensor-Frame rel. Rack
        sPo = self.sPo()
                                        # Obstacle-Position rel. Sensor
        sTo = T3D.FromEuler( 0, sPo.y(), 0)
                                        # Obstacle-Frame rel. Sensor
        rTo = rTs * sTo
        rPo = rTo._P_()
        return rPo
